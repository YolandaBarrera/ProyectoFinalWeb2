...
// require('dotenv').config() permite cargar las variables contenidas
// en el archivo .env (como PORT, DB_HOST, DB_USER, DB_PASSWORD,
// SESSION_SECRET, etc.) para que estén disponibles en process.env.
//
// Esto hace que puedas cambiar configuraciones importantes sin modificar
// el código fuente, manteniendo seguridad y flexibilidad.
//
// La opción "quiet: true" evita que dotenv muestre advertencias o logs.
// Esto es útil cuando no quieres saturar la consola con mensajes si no existe .env.

require('dotenv').config({ quiet: true }); // Carga las variables de entorno desde un archivo .env

// Importa la app de Express y el pool de conexión a MySQL desde app.js
// `app.js` ya tiene configuradas las sesiones, vistas y rutas principales
...
// Destruye la sesión del usuario y responde JSON confirmando la acción
...
// REVISIÓN DE SESIÓN (GET /api/session)
...
app.listen(PORT, () => {
  // Inicia la app Express en el puerto configurado y muestra URL en consola.
